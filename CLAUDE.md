...
- **Live:** https://tictac.tylerrbrown.com/?k=x9f2k7m4-b8c1-e3a5-d6w0-q7r9s2t4u1v8
- **Access key param:** `?k=x9f2k7m4-b8c1-e3a5-d6w0-q7r9s2t4u1v8` (shows "404" without it)

## App Logic

- Tracks timestamps of when user takes a tic tac
- **Green** (ready): >= 3.5 days since last dose — show "take it" button
- **Red** (wait): < 3.5 days — show live countdown to when next dose is due
...
- `GET /api/entries` — list all entries (JSON array of `{id, ts}`)
- `POST /api/entries` — add new entry (returns `{id, ts}`)
